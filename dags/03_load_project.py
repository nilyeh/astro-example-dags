...
def load_orders():
    ...
    if orders_rows>0 :
        ...
        table_id =  "baseheylin.dep_raw.orders"
        job_config = bigquery.LoadJobConfig(
            schema=[
                bigquery.SchemaField("_id", bigquery.enums.SqlTypeNames.STRING),
...
def load_order_items():
    ...
    if order_items_rows>0 :
        ...
        table_id =  "baseheylin.dep_raw.order_items"
        job_config = bigquery.LoadJobConfig(
            schema=[
                bigquery.SchemaField("_id", bigquery.enums.SqlTypeNames.STRING),
                bigquery.SchemaField("order_date", bigquery.enums.SqlTypeNames.DATE),
...
def load_customers():
    ...
    if customers_rows>0 :
        ...
        table_id =  "baseheylin.dep_raw.customers"
        job_config = bigquery.LoadJobConfig(
            schema=[
                bigquery.SchemaField("_id", bigquery.enums.SqlTypeNames.STRING),
...
def load_categories():
    ...
    if categories_rows>0 :
        ...
        table_id =  "baseheylin.dep_raw.categories"
        job_config = bigquery.LoadJobConfig(
            schema=[
...
def load_departments():
    ...
    if departments_rows>0 :
        ...
        table_id =  "baseheylin.dep_raw.departments"
        job_config = bigquery.LoadJobConfig(
            schema=[
...
def load_Tabla_BI():
    ...
    create or replace table `baseheylin.dep_raw.bi_orders` as
    ...
    FROM `baseheylin.dep_raw.master_order` a
    inner join  `baseheylin.dep_raw.products` b on
    a.order_item_product_id=b.product_id
    inner join `baseheylin.dep_raw.categories` c on
    b.product_category_id=c.category_id
    inner join `baseheylin.dep_raw.departments` d on
...
with DAG(
    ...
    step_load_Tabla_BI = PythonOperator(
        task_id='load_Tabla_BI_id',
